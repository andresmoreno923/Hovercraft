...
class ImageOverlay:
    def __init__(self):
        ...
        self.path_resources = os.path.join(self.path_main, 'resources')
        self.color_1 = (0, 0, 255)  # rojo
        self.color_2 = (0, 165, 255)  # naranja
        self.color_3 = (56, 255, 25)  # verde
        self.icon_battery = cv2.imread(os.path.join(
            self.path_resources, 'battery_icon.png'), cv2.IMREAD_UNCHANGED)
        self.icon_hvc = cv2.imread(os.path.join(
            self.path_resources, 'hovercraft_icon.png'), cv2.IMREAD_UNCHANGED)
        self.icon_rssid = cv2.imread(os.path.join(
            self.path_resources, 'rssi_icon.png'), cv2.IMREAD_UNCHANGED)
        self.rssi_mid = -70
        self.rssi_max = -50
    ...
    def draw_rectangle(self, img, data):
        ...
        pt1 = (img_w - 40, int(img_h / 2 - 100))
        pt2 = (img_w - 10, int(img_h / 2 + 100))  # Esquina inferior derecha
        img = cv2.rectangle(img, pt1, pt2, (0, 255, 0),
                            thickness=1, lineType=cv2.LINE_AA)
        cv2.putText(img, '__', (img_w - 40, int(img_h / 2) +
                    data_j), self.font, 1, self.color_3, 1)
        cv2.putText(img, str(-1 * data_j), (img_w - 60,
                    int(img_h / 2) + data_j), self.font, 0.5, self.color_3, 1)
        return img
    ...
    def draw_yaw(self, img, data):
        ...
        for indx in indx_:
            if not (indx % 5):
                cv2.putText(
                    img, '|', (int(idx + n * indx__[i]), 20), self.font, 0.5, self.color_3, 1)
                cv2.putText(img, str(indx), (int(
                    idx + n * indx__[i]), 40), self.font, 0.4, self.color_3, 1)
            i = i + 1
        return img
    ...
    def change_color(self, img, color):
        ...
        green_mask = np.logical_and(
            color_channels[:, :, 1] > 0, color_channels[:, :, 0] > 0, color_channels[:, :, 2] > 0)
    ...
    def rssi(self, txt):
        txt = float(txt)
        if txt >= self.rssi_max:
            color_txt = self.color_3
        elif( txt <= self.rssi_max and txt>= self.rssi_mid):
            color_txt = self.color_2
        else:
            color_txt = self.color_1
        return color_txt
    ...
    def add_transparent_image(self, bg, txt='0', cl=1):
        ...
        if cl == 1:
            ...
            icon = self.icon_hvc
            txt = float(txt)* 2.23694
            txt_1 = 'mph'
            color_txt = self.color_3
            y_offset = bg_w - 150

        elif cl == 2:
            icon = self.icon_rssid
            txt_1 = 'dBm'
            color_txt = self.rssi(txt)
            icon = self.change_color(icon, color_txt)
            y_offset = int(bg_w/2 - 50)

        txt = np.round(float(txt), 2)
        fg_h, fg_w, fg_channels = icon.shape
        x_offset = int(0.90*(bg_h))
        ...
        cv2.putText(bg, str(txt) + str(txt_1), (y_offset + 50,
                    x_offset + 30), self.font, 0.7, color_txt, 1)
    ...
    def add_overlay(self, img, data):
        ...
        img = self.add_transparent_image(img, txt=data[8], cl=2)
